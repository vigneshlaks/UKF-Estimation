...
def initialize_ukf():
    ...
    orientation_process_noise = 1.5
    angular_velocity_process_noise = .08
...
def estimate_rot(data_num=1):
    # Load data
    imu_data = io.loadmat('imu/imuRaw' + str(data_num) + '.mat')
    ...
    accel_calibrated = calibrate_accel(accel, accel_bias, accel_sensitivity)
    gyro_calibrated = calibrate_gyro(gyro, gyro_bias, gyro_sensitivity)
    ...
    for i in range(1, num_steps, 3):
        ...
        predicted_state, predicted_cov = ukf_prediction_step(prev_state,
                                                             current_state,
                                                             current_cov,
                                                             dt,
                                                             process_noise)

        current_state, current_cov = ukf_update_step(predicted_state,
                                                     predicted_cov,
                                                     measurement,
                                                     measurement_noise)
    ...
    min_length = min(len(ukf_timestamps), len(euler_angles))
    ukf_timestamps = ukf_timestamps[:min_length]
    euler_angles = euler_angles[:min_length]

    from scipy.interpolate import interp1d

    roll_interp = interp1d(ukf_timestamps, euler_angles[:, 0],
                           bounds_error=False, fill_value="extrapolate")
    pitch_interp = interp1d(ukf_timestamps, euler_angles[:, 1],
                            bounds_error=False, fill_value="extrapolate")
    yaw_interp = interp1d(ukf_timestamps, euler_angles[:, 2],
                          bounds_error=False, fill_value="extrapolate")

    # Evaluate at all timestamps
    roll = roll_interp(imu_ts)
    pitch = pitch_interp(imu_ts)
    yaw = yaw_interp(imu_ts)
    print(roll)
    print(pitch)
    print(yaw)
